"""System self-diagnostic and health check engine for 'chatprint doctor'."""
...
from chatprint.installer.python_runtime import check_python_version
...
def run_doctor_check() -> DoctorReport:
    ...
    cli_on_path = shutil.which("chatprint") is not None
    checks.append(
        DiagnosticCheck(
            name="CLI PATH",
            status=bool(cli_on_path),
            message="Executable on PATH" if cli_on_path else "Not found on PATH (run via python -m chatprint or install)",
        )
    )
