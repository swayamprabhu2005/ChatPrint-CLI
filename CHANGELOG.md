...
### Added
- Initial release of AI Conversation CLI (`ai-conversation`).
...
- CLI commands:
  - `ai-conversation convert <FILE>` with options `--output`, `--output-file`, `--source`, `--force`, `--debug`.
  - `ai-conversation inspect <FILE>` for diagnostic inspection without PDF generation.
  - `ai-conversation doctor` for system health check.
  - `--version` and `--help`.
